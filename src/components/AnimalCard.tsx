...
export const AnimalCard: React.FC<AnimalCardProps> = ({ animal, isDraggable = true }) => {
  const [isFlipped, setIsFlipped] = useState(false);
  const [hasGuessed, setHasGuessed] = useState(false);
  const [showFamilyModal, setShowFamilyModal] = useState(false);
  const cardRef = useRef<HTMLDivElement>(null);
  const { recordGuess, guessResults } = useAnimalStore();
  ...
  return (
    <div className="relative">
      <div 
        ref={setCardRef}
        className="relative w-80 h-96 cursor-grab active:cursor-grabbing" style={{ opacity: isDragging ? 0.65 : 1 }}
      ...
      >
        ...
        >
          {/* Front of card */}
          <div 
            className="absolute inset-0 w-full h-full backface-hidden bg-white rounded-2xl shadow-lg border-2 border-gray-100 hover:shadow-xl transition-all duration-300"
          ...
          >
            <div className="relative h-full flex flex-col">
              ...
              <div className="text-center px-6 pb-4">
                <h3 className="text-2xl font-bold text-gray-800 mb-6">{animal.name}</h3>
              </div>
          ...
          {/* Back of card */}
          <div 
            className="absolute inset-0 w-full h-full backface-hidden bg-white rounded-2xl shadow-lg border-2 border-gray-100 rotate-y-180"
